...
async function loadGarmentItems() {
  try {
    const [t6Res, t5Res, t4Res, t3Res, t2Res, t1Res, utilityRes, augmentRes,
           ...
           augMeleeRes, augRangedRes, cvarsRes] = await Promise.all([
      ...
      fetch('./data/engine_cvars.json'),
    ]);
    ENGINE_CVAR_CATALOG = await cvarsRes.json();
...
function getCvarCatalogEntry(name) {
  if (!ENGINE_CVAR_CATALOG) return null;
  return ENGINE_CVAR_CATALOG.cvars.find(c => c.name.toLowerCase() === name.toLowerCase()) || null;
}
...
function renderEngineIniAvailable(searchTerm = '') {
  ...
  for (const cat of ENGINE_CVAR_CATALOG.categories) {
    ...
    const catCvars = ENGINE_CVAR_CATALOG.cvars.filter(c => c.category === cat.id);
...
async function openEngineIniModal() {
  if (!ENGINE_CVAR_CATALOG) {
    showError('Engine cvar catalog not loaded.');
    return;
  }
  closeSettings();
